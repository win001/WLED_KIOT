...
void getSupportedConfigWeb(JsonBuffer& jsonBuffer, JsonObject& root) {
    // DynamicJsonBuffer jsonBuffer;
    // JsonObject& root = jsonBuffer.createObject();

    for (int i = 0; i < set_config_keys_callbacks.size(); i++) {
        // JsonObject& rootTemp = jsonBuffer.createObject();
        char topic[MAX_CONFIG_TOPIC_SIZE] = "";
        (set_config_keys_callbacks[i])(jsonBuffer, root, topic, true);
        // root[String(topic)] = rootTemp;

        // JsonObject& data = root.createNestedObject(String(topic));
        // cloneJsonVariant(jsonBuffer);
    }
}
...
#if (RELAY_PROVIDER == RELAY_PROVIDER_ESP) || (RELAY_PROVIDER == RELAY_PROVIDER_KU)
    uint8_t status = EEPROMr.read(EEPROM_RELAY_STATUS);

    int _dimmerCount = dimmerCount();
    int dimmer_status[_dimmerCount];
    if (_dimmerCount > 0) {
        for (int i = 0; i < _dimmerCount; i++) {
            dimmer_status[i] = getSetting("dim", i , "99").toInt();
        }
    }
#endif

#if defined(SMART_DONGLE)
    String deviceId2 = getSetting("deviceId2");
#endif
...
#if (RELAY_PROVIDER == RELAY_PROVIDER_ESP) || (RELAY_PROVIDER == RELAY_PROVIDER_KU)
    EEPROMr.write(EEPROM_RELAY_STATUS, status);

    if (_dimmerCount > 0) {
        for (int i = 0; i < _dimmerCount; i++) {
            setSetting("dim", i , dimmer_status[i]);
        }
    }    
#endif

#if RELAY_PROVIDER == RELAY_PROVIDER_LIGHT
    setSetting("LSW",LEDSHOW);
#endif

#if defined(SMART_DONGLE)
    setSetting("deviceId2", deviceId2);
#endif
    // DEBUG_MSG_P(PSTR("[FACTORYSET] EEPROM RELAY STATUS - %d \n"), EEPROMr.read(EEPROM_RELAY_STATUS));

    saveSettings();

    deferredReset(1000, CUSTOM_RESET_FACTORY);

    #if RELAY_PROVIDER == RELAY_PROVIDER_LIGHT
        // Special Case - We want the light to be turned on after factoryset.
        // Because otherwise pattern will also not be played and users wont undersatnd. 
        // And the problem is deferedReset() is also calling relaySave(). So we just have to override it. 
...
void settingsLoop()
{
...
#if KU_SERIAL_SUPPORT && (ENCRYPT_CLI == 0)
    if (Serial.available() && Serial.peek() == 0x55)
    {
        KUSerialprocess();
    }
    
    // Asks for product info every PRODUCT_INFO_QUERY_INTERVAL milli seconds.
    static unsigned long lastSentTime = 0;

	if(areRealysConfigured() == false && millis() - lastSentTime >= PRODUCT_INFO_QUERY_INTERVAL)
	{
		lastSentTime += PRODUCT_INFO_QUERY_INTERVAL;
        KUQueryProuctInfoChild();
	}

#endif
#if TUYA_SUPPORT
    // If Serial Command Starts with 0x55 hen skip this one from embedis and give tuya loop a chance to process this command.
    if (Serial.available())
    {
        if (Serial.peek() == 0x55)
        {
    ...
    if (millis() - last_settings_sent > SETTINGS_UPDATE_TIME || millis() < last_settings_sent)
    {
...
void settingsMQTTCallback(unsigned int type, const char* topic, const char* payload, bool retained, bool isEncrypted) {
